...
export const PROJECTS: Project[] = [
  // ---- Perceptual & imaging phenotyping ----
  {
    ...
    summary:
      "Perceptual skin-tone phenotyping over ~17.8K open dermatology images (Fitzpatrick17k, ISIC 2018). A layered masking pipeline — foreground segmentation plus a ResNet18-U-Net lesion mask (held-out Dice 0.889) — isolates skin before CIE Lab featurization and CIEDE2000 perceptual clustering. Measured colour barely tracks Fitzpatrick type: type explains 7% of lightness variance, and predicting it from colour barely beats guessing the commonest type (35–42% against 34%); masking did not help. An earlier claim that discovered clusters were 2.5–2.8× more predictable was withdrawn as largely circular; a pre-registered retest found no stable colour categories at all (resampling ARI 0.31). Against a colorimeter on a second dataset, Fitzpatrick type tracks skin colour strongly and Monk Skin Tone better, while image-derived colour does not: the weak link was the camera, not the scale.",
  },
  {
